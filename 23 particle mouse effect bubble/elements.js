...
class Particle {
    ...
    update(mouse) {
        ...
        this.draw(dist < maxDist);
    }
    draw(checkMaxDist) {
        if(checkMaxDist){

            drawCircle(this.ctx, this.x, this.y, this.size, this.color, true);
            drawCircle(this.ctx, this.x - 5, this.y - 5, this.size / 4, this.color);
            drawCircle(this.ctx, this.x + 4, this.y - 5, this.size / 7, this.color);
        }else{
            drawCircle(this.ctx, this.x, this.y, this.size, this.color, true);
            drawCircle(this.ctx, this.x , this.y , this.size / 5, this.color);
        }
    }
}
